            ...
            if len(self.img)!= 0:
                ...
                if len(self.obstacle_list) > 0:
                    ...
                    for obstacle in self.obstacle_list:
                        ...
                        # 오른쪽뒤 체크
                        if -3 <= obstacle.x <= -0.8 and -3.5 <= obstacle.y <= 0:
                            tunnel_statlc_roi_check_arr[2] = 1
                        #장애물과 라이다의 상대각도 계산
                        theta = math.degrees(math.atan2(obstacle.y, obstacle.x))
                        self.theta_list.append(theta)

                        print("Theta: ", theta)

                    # tunnel_statlc_roi_check_arr 해석
                    if (tunnel_statlc_roi_check_arr == [1, 1, 0]) or (tunnel_statlc_roi_check_arr == [1, 1, 1]):
