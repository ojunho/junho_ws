...
void cfgCallback(lidar_object_detection::objectDetectorLatticeConfig &config_lattice, int32_t level) {
    xMinROI = config_lattice.xMinROI;
    xMaxROI = config_lattice.xMaxROI;
    yMinROI = config_lattice.yMinROI;
    yMaxROI = config_lattice.yMaxROI;
    zMinROI = config_lattice.zMinROI;
    zMaxROI = config_lattice.zMaxROI;

    minPoints = config_lattice.minPoints;
    epsilon = config_lattice.epsilon;
    minClusterSize = config_lattice.minClusterSize;
    maxClusterSize = config_lattice.maxClusterSize;

    xMinBoundingBox = config_lattice.xMinBoundingBox;
    xMaxBoundingBox = config_lattice.xMaxBoundingBox;
    yMinBoundingBox = config_lattice.yMinBoundingBox;
    yMaxBoundingBox = config_lattice.yMaxBoundingBox;
    zMinBoundingBox = config_lattice.zMinBoundingBox;
    zMaxBoundingBox = config_lattice.zMaxBoundingBox;

    leafSize  = config_lattice.leafSize;

    maxIterations = config_lattice.maxIterations;
    distanceThreshold = config_lattice.distanceThreshold;
}
...
int main (int argc, char** argv) {
    // Initialize ROS
    ros::init (argc, argv, "lidar_object_detection_lattice");
    ros::NodeHandle nh;

    dynamic_reconfigure::Server<lidar_object_detection::objectDetectorLatticeConfig> server;
    dynamic_reconfigure::Server<lidar_object_detection::objectDetectorLatticeConfig>::CallbackType f;
    ...
    pubROI = nh.advertise<sensor_msgs::PointCloud2> ("roi_raw_lattice", 1);
    pubCluster = nh.advertise<sensor_msgs::PointCloud2>("cluster_lattice", 1);
    pubObjectInfo = nh.advertise<lidar_object_detection::ObjectInfo>("object_info_lattice", 1);
    pubPointInfo = nh.advertise<lidar_object_detection::PointInfo>("bbox_point_info_lattice", 1);
    pubObjectShapeArray = nh.advertise<visualization_msgs::MarkerArray>("bounding_box_lattice", 1);
    pubSegmentPlaneInfo = nh.advertise<sensor_msgs::PointCloud2> ("segment_plane_lattice", 1);
