...
void cfgCallback(lidar_object_detection::objectDetectorDynamicObstacleConfig &config_dynamic_obstacle, int32_t level) {
    xMinROI = config_dynamic_obstacle.xMinROI;
    xMaxROI = config_dynamic_obstacle.xMaxROI;
    yMinROI = config_dynamic_obstacle.yMinROI;
    yMaxROI = config_dynamic_obstacle.yMaxROI;
    zMinROI = config_dynamic_obstacle.zMinROI;
    zMaxROI = config_dynamic_obstacle.zMaxROI;

    minPoints = config_dynamic_obstacle.minPoints;
    epsilon = config_dynamic_obstacle.epsilon;
    minClusterSize = config_dynamic_obstacle.minClusterSize;
    maxClusterSize = config_dynamic_obstacle.maxClusterSize;

    xMinBoundingBox = config_dynamic_obstacle.xMinBoundingBox;
    xMaxBoundingBox = config_dynamic_obstacle.xMaxBoundingBox;
    yMinBoundingBox = config_dynamic_obstacle.yMinBoundingBox;
    yMaxBoundingBox = config_dynamic_obstacle.yMaxBoundingBox;
    zMinBoundingBox = config_dynamic_obstacle.zMinBoundingBox;
    zMaxBoundingBox = config_dynamic_obstacle.zMaxBoundingBox;

    leafSize  = config_dynamic_obstacle.leafSize;

    maxIterations = config_dynamic_obstacle.maxIterations;
    distanceThreshold = config_dynamic_obstacle.distanceThreshold;
}
...
int main (int argc, char** argv) {
    // Initialize ROS
    ros::init (argc, argv, "lidar_object_detection_dynamic_obstacle");
    ros::NodeHandle nh;

    dynamic_reconfigure::Server<lidar_object_detection::objectDetectorDynamicObstacleConfig> server;
    dynamic_reconfigure::Server<lidar_object_detection::objectDetectorDynamicObstacleConfig>::CallbackType f;
    ...
    pubROI = nh.advertise<sensor_msgs::PointCloud2> ("roi_raw_dynamic_obstacle", 1);
    pubCluster = nh.advertise<sensor_msgs::PointCloud2>("cluster_dynamic_obstacle", 1);
    pubObjectInfo = nh.advertise<lidar_object_detection::ObjectInfo>("object_info_dynamic_obstacle", 1);
    pubPointInfo = nh.advertise<lidar_object_detection::PointInfo>("bbox_point_info_dynamic_obstacle", 1);
    pubObjectShapeArray = nh.advertise<visualization_msgs::MarkerArray>("bounding_box_dynamic_obstacle", 1);
    pubSegmentPlaneInfo = nh.advertise<sensor_msgs::PointCloud2> ("segment_plane_dynamic_obstacle", 1);
